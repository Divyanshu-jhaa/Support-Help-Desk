...
const Login = (props) => {
    ...
    const handleLogin = async (data) => {
        try {
            const res = await axios.post("http://localhost:5000/api/v1/users/getUserByAuth", data);
            if (res.data.token) {
                localStorage.setItem('user', JSON.stringify(res.data));

                if (res.data.designation === '0') {
                    naviagte("/user");
                } else if (res.data.designation === '1') {
                    naviagte("/usersupport");
                } else {
                    naviagte("/admin");
                }
            } else {
